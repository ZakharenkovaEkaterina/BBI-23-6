...
class Program
{
    static void Main()
    {
        ...
        //    Console.WriteLine();
        //}
        //Skater.CalculatePlaceSum(skaters);
    ...
    class Skater //2 уровень 9 задача
    {
        ...
        public static void CalculatePlaceSum(Skater[] skaters)
        {
            for (int i = 0; i < skaters.Length; i++)
            {
                ...
                {
                    double place = 1;
                    for (int k = 0; k < skaters.Length; k++)
                    {
                        if (k != i && skaters[i]._results[j] > skaters[k]._results[j])
                        {
                            place++;
                        }
                    }
                ...
                skaters[i]._placeSum = placeSum;
            }
        }
        public static void Sort(Skater[] skaters)
        {
            for (int i = 1; i < skaters.Length; i++)
            {
                for (int j = i; j > 0 && skaters[j]._placeSum > skaters[j - 1]._placeSum; j--)
                {
                    var tmp = skaters[j];
